...
class _ProfileScreenState extends State<Profile> {
  ...
  Future<void> downloadCsv() async {
    final url = 'https://afternoon-mountain-03139-5af4f0a20604.herokuapp.com/api/export-csv';
    ...
    if (status.isGranted) {
      ...
      if (response.statusCode == 200) {
        ...
        final directory = Directory('/storage/emulated/0/Yeet/Internal storage'); // Update to the desired path
        // Create the directory if it doesn't exist
        if (!(await directory.exists())) {
          await directory.create(recursive: true); // Create the directory if it doesn't exist
        }

        final filePath = '${directory.path}/student_bag_items_export.csv';
        ...
        // Handle the error
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to download CSV: ${response.statusCode}')),
        );
      }
    } else {
                  ...
                  return [
                    SliverAppBar(
                      ...
                      actions: <Widget>[
                        ...
                        IconButton(
                          icon: Icon(Icons.download, color: primary_color), // Download icon
                          onPressed: () {
                            downloadCsv();
                          },
                        ),
